...
The main launch file accepts a `maze` argument to select which maze environment to load:

```bash
ros2 launch gazebo_controller full_simulation.launch.py maze:=<maze_name>
```
...
# Launch with Maze_hr
ros2 launch gazebo_controller full_simulation.launch.py maze:=Maze_hr

# Launch with Maze_ng
ros2 launch gazebo_controller full_simulation.launch.py maze:=Maze_ng
```
...
4. **RViz** - Visualization with:
   ...
   - Goal point markers
   - TF frames
5. **Controller Node** - PID-based differential drive controller
6. **Publishers** - Map and goal point publishers
